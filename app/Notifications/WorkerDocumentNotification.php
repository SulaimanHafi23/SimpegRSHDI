...
class WorkerDocumentNotification extends Notification implements ShouldQueue
{
    ...
    private function getSubject(string $documentName): string
    {
        return match ($this->action) {
            'verified' => "[SIMPEG] Dokumen {$documentName} Anda Telah Diverifikasi",
            'rejected' => "[SIMPEG] Dokumen {$documentName} Anda Ditolak",
            default    => "[SIMPEG] Update Status Dokumen {$documentName}",
        };
    }
