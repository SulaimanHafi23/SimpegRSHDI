...
class ShiftSwapExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize
{
    protected $swaps;

    public function __construct(Collection $swaps)
    {
        $this->swaps = $swaps;
    }
    ...
    public function map($swap): array
    {
        ...
        return [
            ...
            $swapType,
            $swap->requester->name ?? '-',
            $swap->requester->department->name ?? '-',
            $swap->requesterShift->shift->name ?? '-',
            $swap->targetWorker->name ?? '-',
            $swap->targetWorker->department->name ?? '-',
            $swap->targetShift->shift->name ?? '-',
            $status,
            $swap->manager ? \App\Models\User::find($swap->manager_id)?->name ?? '-' : '-',
            $swap->manager_approved_at?->format('d/m/Y') ?? '-',
            $swap->executed_at?->format('d/m/Y') ?? '-',
            $swap->reason ?? '-',
        ];
    }
