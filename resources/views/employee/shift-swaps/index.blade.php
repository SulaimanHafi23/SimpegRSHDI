...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-green-600 to-green-700 rounded-lg shadow-lg p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <x-export-buttons :route="route('employee.shift-swaps.export')" title="Export Tukar Shift">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Partner / Rekan</label>
                            <select name="partner_id"
                                ...
                                <option value="">Semua Rekan</option>
                                @php
                                    $partners = collect();
                                    foreach ($items as $item) {
                                        if ($item->requester_id === auth()->user()->worker->id && $item->targetWorker) {
                                            $partners->push($item->targetWorker);
                                        } elseif ($item->requester) {
                                            $partners->push($item->requester);
                                        }
                                    }
                                    $partners = $partners->unique('id')->sortBy('name');
                                @endphp
                    ...
                    <a href="{{ route('employee.shift-swaps.create') }}"
                        class="inline-flex items-center px-6 py-3 bg-white text-green-700 font-semibold rounded-lg shadow-md hover:bg-green-50 transition duration-200">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <div
                class="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200 p-5 border-2 border-blue-200">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <div class="bg-white rounded-lg shadow-md p-4 flex flex-wrap items-center gap-2 mb-2">
                ...
                <button @click="filter = 'open'"
                    ...
                    @if(($summary['open_requests'] ?? 0) > 0)
                        <span
                            class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{{ $summary['open_requests'] }}</span>
                    @endif
                </button>
                ...
                @if(isset($openRequests) && $openRequests->count() > 0)
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                        <div class="flex items-center mb-3">
                            ...
                            <span
                                class="ml-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-full">{{ $openRequests->count() }}</span>
                        </div>
                                ...
                                <div class="bg-white rounded-lg shadow-sm border border-blue-100 p-4 hover:shadow-md transition">
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-sm text-gray-600">
                                                    {{ $openRequest->requester->department->name ?? '-' }}
                                                </p>
                                                <div class="mt-2 text-sm text-gray-500">
                                                    <i class="fas fa-clock mr-1"></i>
                                                    <span
                                                        class="font-medium">{{ $openRequest->requesterShift?->shift->name ?? 'N/A' }}</span>
                                                    ...
                                                    @elseif($openRequest->swap_type === 'date_range' && $openRequest->swap_start_date && $openRequest->swap_end_date)
                                                        {{ $openRequest->swap_start_date->format('d M Y') }} s/d
                                                        {{ $openRequest->swap_end_date->format('d M Y') }}
                                                    @else
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <span
                                                class="text-xs text-gray-500">{{ $openRequest->created_at->diffForHumans() }}</span>
            ...
            <div class="space-y-4" x-show="filter !== 'open'">
                @forelse($items as $item)
                    @php
                        ...
                        $statusConfig = [
                            'pending' => ['color' => 'yellow', 'icon' => 'clock', 'text' => 'Menunggu Partner'],
                            'accepted' => ['color' => 'green', 'icon' => 'check', 'text' => 'Diterima Partner'],
                            'awaiting_approval' => ['color' => 'blue', 'icon' => 'hourglass-half', 'text' => 'Menunggu Manager'],
                            'manager_verified' => ['color' => 'indigo', 'icon' => 'check-double', 'text' => 'Terverifikasi (Menunggu HR)'],
                    ...
                    <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition duration-200 overflow-hidden"
                        data-status="{{ $item->status }}" x-cloak
                        x-show="filter === 'all'
                        ...
                        <div
                            class="bg-{{ $status['color'] }}-50 border-l-4 border-{{ $status['color'] }}-500 px-4 sm:px-6 py-3">
                            <div class="flex items-center justify-between">
                                ...
                                <span
                                    class="text-sm text-{{ $status['color'] }}-600">{{ $item->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 sm:p-6">
                                    ...
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="font-semibold text-gray-900 truncate">
                                                {{ $item->targetWorker?->name ?? 'Open Request' }}
                                            </p>
                                            <p class="text-sm text-gray-600 truncate">
                                                {{ $item->targetWorker?->department->name ?? '-' }}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div class="mt-4 p-3 bg-indigo-50 border-l-4 border-indigo-400 rounded">
                                ...
                                @elseif($item->swap_type === 'date_range' && $item->swap_start_date && $item->swap_end_date)
                                    <p class="text-sm text-indigo-900 font-semibold">
                                        {{ $item->swap_start_date->format('d M Y') }} s/d
                                        {{ $item->swap_end_date->format('d M Y') }}
                                    </p>
                                @elseif($item->swap_type === 'recurring' && $item->swap_dates)
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        @foreach($item->swap_dates as $swapDate)
                                            <span
                                                class="inline-flex items-center px-2 py-1 bg-indigo-100 text-indigo-800 text-xs font-medium rounded">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                @if($isTarget && $item->status === 'pending')
                                    <form action="{{ route('employee.shift-swaps.accept', $item->id) }}" method="POST"
                                        class="inline w-full sm:w-auto">
                                        @csrf
                                        <button type="submit"
                                            class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition duration-200 shadow-sm">
                                            <i class="fas fa-check mr-2"></i>
                                            Terima
                                        </button>
                                    </form>
                                    <button type="button" onclick="rejectSwap('{{ $item->id }}')"
                                ...
                                @if($isRequester && !in_array($item->status, ['executed', 'cancelled']))
                                    <form action="{{ route('employee.shift-swaps.cancel', $item->id) }}" method="POST"
                                        class="inline w-full sm:w-auto">
                                        @csrf
                                        <button type="submit"
    ...
    <!-- Reject Modal -->
    <div class="fixed inset-0 backdrop-blur-sm bg-white/30 z-50 hidden" id="rejectModal"
                ...
                <form id="rejectForm" method="POST">
                    ...
                    <div class="p-6">
                        ...
                        <textarea name="reason"
                            ...
                            rows="4" placeholder="Tuliskan alasan penolakan Anda..."></textarea>
                    ...
                    <div class="bg-gray-50 px-6 py-4 rounded-b-lg flex gap-3">
                        <button type="submit"
                            class="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition duration-200 shadow-sm">
                            <i class="fas fa-times mr-2"></i>
                            Tolak
                        </button>
                        <button type="button" onclick="closeRejectModal()"
                            class="flex-1 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg transition duration-200">
                            Batal
                        </button>
                    </div>
                </form>
