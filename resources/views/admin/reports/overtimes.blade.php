...
<div class="space-y-6">
    <div>
        ...
        <p class="text-gray-600">Ringkasan permohonan lembur berdasarkan rentang tanggal dan status.</p>
    </div>

    <!-- Filter -->
    <div class="bg-white rounded-lg shadow p-6">
        <form method="GET" action="{{ route('reports.overtimes') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end justify-end gap-2">
                ...
                <a href="{{ route('reports.overtimes.export', request()->only(['start_date','end_date','status','worker_id'])) }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Export Excel</a>
            </div>
        </form>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $ot->overtime_date->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $ot->worker->name ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ optional($ot->start_time)->format('H:i') ?? '-' }} - {{ optional($ot->end_time)->format('H:i') ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $ot->total_hours }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ ucfirst($ot->status) }}</td>
