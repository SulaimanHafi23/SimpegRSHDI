...
<div class="space-y-4 sm:space-y-6">
    <div class="flex items-center space-x-4">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Jabatan</h1>
            <p class="text-sm text-gray-600 mt-1">Ubah data jabatan</p>
        </div>
    </div>

    <div class="bg-white rounded-lg shadow">
        <form action="{{ route('admin.master.positions.update', $position->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div>
