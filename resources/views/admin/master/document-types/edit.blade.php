        ...
        <form action="{{ route('admin.master.document-types.update', $documentType->id) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="employment_category" id="employment_category" class="w-full px-3 py-2 border rounded-lg" required>
                            @foreach($employmentCategories as $key => $label)
                                <option value="{{ $key }}" {{ old('employment_category', $documentType->employment_category ?? 'all') === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="process_type" id="process_type" class="w-full px-3 py-2 border rounded-lg" required>
                            @foreach($processTypes as $key => $label)
                                <option value="{{ $key }}" {{ old('process_type', $documentType->process_type ?? 'onboarding') === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="source_document_type_id" id="source_document_type_id" class="w-full px-3 py-2 border rounded-lg">
                            ...
                            @foreach($baseDocumentTypes as $baseDoc)
                                <option value="{{ $baseDoc->id }}" {{ old('source_document_type_id', $documentType->source_document_type_id) === $baseDoc->id ? 'selected' : '' }}>{{ $baseDoc->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="expiration_buffer_days" class="block text-sm font-medium text-gray-700 mb-2">Buffer Kadaluarsa (hari)</label>
                        <input type="number" name="expiration_buffer_days" id="expiration_buffer_days" min="0" max="365" value="{{ old('expiration_buffer_days', $documentType->expiration_buffer_days ?? 0) }}" class="w-full px-3 py-2 border rounded-lg" />
                    </div>
                    <div class="md:col-span-2">
                        <label for="requirement_notes" class="block text-sm font-medium text-gray-700 mb-2">Catatan Aturan</label>
                        <textarea name="requirement_notes" id="requirement_notes" rows="2" class="w-full px-3 py-2 border rounded-lg">{{ old('requirement_notes', $documentType->requirement_notes) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-3">
                <label class="flex items-center space-x-2">
                    <input type="checkbox" name="is_required" value="1" {{ old('is_required', $documentType->is_required) ? 'checked' : '' }} class="rounded">
