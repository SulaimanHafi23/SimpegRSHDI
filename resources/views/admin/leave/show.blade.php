            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    @php
                        ...
                        $config = $statusConfig[$leave->status] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-800', 'icon' => ''];
                    @endphp
                    ...
                    <div>
                        ...
                        <p class="text-xl sm:text-2xl font-bold {{ $config['text'] }}">{{ $leave->status }}</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 space-y-6">
                    ...
                    <div class="flex items-start space-x-4">
                        @if($leave->worker->photo)
                            <img class="h-20 w-20 rounded-lg object-cover" 
                                 src="{{ Storage::url($leave->worker->photo) }}" 
                        ...
                        @endif
                        <div class="flex-1">
                            <h3 class="text-base sm:text-lg font-semibold text-gray-800">{{ $leave->worker->name }}</h3>
                            <p class="text-gray-600">{{ $leave->worker->position->name ?? '-' }}</p>
                            <div class="mt-2 space-y-1">
                                <p class="text-sm text-gray-600">
                                    <span class="font-medium">NIP:</span> {{ $leave->worker->employee_number }}
                                </p>
                                <p class="text-sm text-gray-600">
                                    <span class="font-medium">Email:</span> {{ $leave->worker->email }}
                                </p>
                                <p class="text-sm text-gray-600">
                                    <span class="font-medium">Phone:</span> {{ $leave->worker->phone }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between py-3 border-b border-gray-200">
                            ...
                            <span class="text-gray-800 font-semibold">{{ $leave->start_date->format('d F Y') }}</span>
                        </div>
                        <div class="flex justify-between py-3 border-b border-gray-200">
                            <span class="text-gray-600 font-medium">Tanggal Selesai</span>
                            <span class="text-gray-800 font-semibold">{{ $leave->end_date->format('d F Y') }}</span>
                        </div>
                        <div class="flex justify-between py-3 border-b border-gray-200">
                            <span class="text-gray-600 font-medium">Total Hari</span>
                            <span class="text-gray-800 font-bold text-lg">{{ $leave->total_days }} hari</span>
                        </div>
                        <div class="py-3">
                            <span class="text-gray-600 font-medium block mb-2">Alasan</span>
                            <p class="text-gray-800 bg-gray-50 p-4 rounded-md">{{ $leave->reason }}</p>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <p class="text-gray-800 bg-yellow-50 p-4 rounded-md border-l-4 border-yellow-400">{{ $leave->notes }}</p>
                </div>
                @endif
            </div>
            ...
            <div class="space-y-4 sm:space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3 flex-1">
                                ...
                                @if($leave->approver)
                                <p class="text-xs text-gray-600 mt-1">oleh {{ $leave->approver->name }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
                ...
                <div class="bg-gradient-to-br from-green-500 to-green-700 rounded-lg shadow-md p-6 text-white">
                    ...
                    <div class="text-center">
                        <p class="text-5xl font-bold">{{ $leave->total_days }}</p>
