        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($workerShifts as $workerShift)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $workerShift->worker->name }}</div>
                                    <div class="text-sm text-gray-500">{{ $workerShift->worker->employee_number ?? '-' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $workerShift->shift->name }}</div>
                            <div class="text-sm text-gray-500">{{ $workerShift->shift->code ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                {{ $workerShift->shift->start_time }} - {{ $workerShift->shift->end_time }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                ...
                                $config = $patternConfig[$workerShift->pattern_type] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-800', 'label' => $workerShift->pattern_type];
                            @endphp
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $workerShift->effective_from->format('d M Y') }}</div>
                            <div class="text-sm text-gray-500">
                                s/d {{ $workerShift->effective_until ? $workerShift->effective_until->format('d M Y') : 'Selamanya' }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                <a href="{{ route('admin.worker-shifts.show', $workerShift->id) }}" 
                                ...
                                <form action="{{ route('admin.worker-shifts.destroy', $workerShift->id) }}" 
                                      method="POST" 
                                      class="inline-block"
                                      onsubmit="return confirm('Are you sure you want to delete this worker shift?');">
                                    @csrf
                                    @method('DELETE')
