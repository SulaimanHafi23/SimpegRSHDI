...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Approval Cuti</h1>
            <p class="text-sm sm:text-base text-gray-600 mt-1">Kelola pengajuan cuti pegawai</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="GET" action="{{ route('approvals.leaves.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2 flex items-end gap-2">
                ...
                <a href="{{ route('approvals.leaves.index') }}" class="flex-1 sm:flex-none px-4 sm:px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Tipe Cuti</th>
                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Tanggal</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-3 sm:px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3 sm:ml-4">
                                    <div class="text-xs sm:text-sm font-medium text-gray-900">{{ $leave->worker->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $leave->worker->nip }}</div>
                                    @if($leave->worker->department)
                                    <div class="text-xs text-gray-400 hidden md:block">{{ $leave->worker->department->name }}</div>
                                    @endif
                                    <div class="text-xs text-blue-600 md:hidden">{{ $leave->leaveType->name }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3 sm:px-6 py-4 hidden lg:table-cell text-xs sm:text-sm text-gray-600">
                        ...
                        <td class="px-3 sm:px-6 py-4 hidden lg:table-cell">
                            <span class="text-xs sm:text-sm font-semibold text-gray-900">{{ $leave->total_days }}</span>
