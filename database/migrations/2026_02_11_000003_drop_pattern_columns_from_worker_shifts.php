...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('worker_shifts', function (Blueprint $table) {
            if (Schema::hasColumn('worker_shifts', 'pattern_type')) {
                $table->dropColumn('pattern_type');
            }
            if (Schema::hasColumn('worker_shifts', 'rotating_days')) {
                $table->dropColumn('rotating_days');
            }
    ...
    public function down(): void
    {
        Schema::table('worker_shifts', function (Blueprint $table) {
            if (!Schema::hasColumn('worker_shifts', 'pattern_type')) {
                $table->enum('pattern_type', ['fixed', 'rotating', 'custom'])->default('fixed')->after('shift_id');
            }
            if (!Schema::hasColumn('worker_shifts', 'rotating_days')) {
                $table->json('rotating_days')->nullable()->after('pattern_type');
            }
            if (!Schema::hasColumn('worker_shifts', 'custom_working_days')) {
